...
if grep -q -e "\$DEFAULT_PDF_SETS" -e "\$DEFAULT_PDF_MEMBERS" $CARDSDIR/${name}_run_card.dat; then
    ...
    if [ "$isnlo" -gt "0" ]; then
        if [ $is5FlavorScheme -eq 1 ]; then
            # 5F PDF
                  sed "s/\$DEFAULT_PDF_SETS/315200,306000,322500,322700,322900,323100,323300,323500,323700,323900,305800,13000,13065,13069,13100,13163,13167,13200,25200,25300,25000,42780,90200,91200,90400,91400,61100,61130,61200,61230,13400,82200,292200,292600,315000,262000,263000/" $CARDSDIR/${name}_run_card.dat > ./Cards/run_card.dat
